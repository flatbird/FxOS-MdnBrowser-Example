document.addEventListener('DOMContentLoaded', function () {
  ...
  // When the browser starts loading content, we switch the "Go" and "Stop"
  // buttons
  browser.addEventListener('mozbrowserloadstart', function () {
    uiLoading(true);
  });

  // When the browser finishes loading content, we switch back the "Go" and
  // "Stop" buttons
  ...
  browser.addEventListener('mozbrowsererror', function (event) {
    uiLoading(false);
    global.alert('Loading error: ' + event.detail);
  });

  // When a user follows a link, we make sure the new location is displayed
  // in the address bar
